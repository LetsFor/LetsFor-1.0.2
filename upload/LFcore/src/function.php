...
class views
{
	public function handleForumViews($id) {
		...
		$ipCheckUser = mfa(dbquery("SELECT * FROM `tems_vievs_ip` WHERE `ip` = '" . $user['ip'] . "'"));
		...
		if(isset($user['id'])) {
			...
			if ($ipCheckUser['user_v'] == $user['id'] && $ip_viev == $ipCheckUser['ip']) {
				...
				if ($ipCheckUser['ip'] != $user['ip']) {
					dbquery("INSERT INTO `tems_vievs_ip` SET `user_v` = '" . $user['id'] . "', `tema` = '" . $forum['id'] . "', `ip` = '" . $ip_viev . "' LIMIT 1");
				}
			}
			
			// 2. Обновление userID и IP
			if ($user['ip'] == $ipCheckUser['ip']) {
				dbquery("UPDATE `tems_vievs_ip` SET `user_v` = '" . $user['id'] . "', `ip` = '" . $user['ip'] . "' WHERE `ip` = '" . $user['ip'] . "'");
			}
